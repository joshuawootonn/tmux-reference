...
<br />
For this to be helpful your need to know what the next session is hard to find
if you rename sessions because `ctrl + b` + `s` orders sessions by name.

To order your sessions by index instead you can use add this snippet to your `~/tmux.conf`
...
Goto prev/next window (VIM style)
...
- <code class="px-0">-s</code> = source
- <code class="px-0">-t</code> = target
...
Goto pane in direction (VIM style)
